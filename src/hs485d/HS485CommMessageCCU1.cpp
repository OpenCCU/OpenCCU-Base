...
HS485CommMessageCCU1::HS485CommMessageCCU1(void)
:HS485CommMessage()
{
...
int HS485CommMessageCCU1::MapIndex(int index)
{
	switch(GetCommand()){
		case CMD_SEND:
...
int HS485CommMessageCCU1::GetCtrl()
{
...
void HS485CommMessageCCU1::SetCtrl(int flags)
{
...
void HS485CommMessageCCU1::SetFrame(const StructuredFrame& frame)
{
...
HS485Frame HS485CommMessageCCU1::ExtractFrame()
{
    HS485Frame frame;
    switch(GetCommand()){
        case CMD_RESPONSE:
...
void HS485CommMessageCCU1::SetTimeout(int to)
{
	if(GetCommand()==CMD_SEND || GetCommand()==CMD_BOOT_SEND)SetByteData(0, to);
}

bool HS485CommMessageCCU1::ProcessResponse(CommMessage *m, t_state* new_state)
{
	if(eepRomUsage) {
...
bool HS485CommMessageCCU1::TransformToSimulationMessage()
{
...
bool HS485CommMessageCCU1::ProcessEEPRomResponse(CommMessage* m, t_state* new_state)
{//Content copied from HS485CommMessageEEPromUsage.cpp
	HS485CommMessage* hs485_msg=(HS485CommMessage*)m;
    ...
    if(state==WAIT_ACK){
		if( (hs485_msg->GetCommand()==CMD_RESPONSE && hs485_msg->GetID()==GetID()) ||
			(hs485_msg->GetCommand()==CMD_EVENT && hs485_msg->GetSenderAddress()==GetReceiverAddress()))
		{
			...
			if(response_payload.size()<(unsigned int)((my_payload[IDX_COUNT]+7)/8)+4){
				LOG(Logger::LOG_ERROR, "HS485CommMessageEEPromUsage::ProcessResponse(): response size mismatch");
			    pthread_mutex_unlock(&mutex);
			    return false;
			}
			if(response_payload[IDX_COMMAND]!='e'){
				LOG(Logger::LOG_ERROR, "HS485CommMessageEEPromUsage::ProcessResponse(): response command mismatch");
			    pthread_mutex_unlock(&mutex);
			    return false;
			}
			if(response_payload.substr(IDX_ADDRESS, 3) != my_payload.substr(IDX_ADDRESS, 3)){
				LOG(Logger::LOG_ERROR, "HS485CommMessageEEPromUsage::ProcessResponse(): response range mismatch");
			    pthread_mutex_unlock(&mutex);
			    return false;
			}
			m->SetParent(this);
