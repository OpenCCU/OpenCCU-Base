...
CCU2SerialPortWrapperWin32::CCU2SerialPortWrapperWin32()
...
int CCU2SerialPortWrapperWin32::SendData(const std::string& data)
{
	/*
#ifdef DUMP
    printf("send:\n");
	...
	return count;
	*/
	//LOG(Logger::LOG_ALL, " TX FRAME: %s", toDebugHexStr(data).c_str()); 
...
int CCU2SerialPortWrapperWin32::ReadData(std::string* data)
{
	if(serialPortHandle == NULL) {
		return 0;
	}
...
bool CCU2SerialPortWrapperWin32::Open(std::string dev)
{
	if(serialPortHandle != NULL) {
		Close();
	}
...
void CCU2SerialPortWrapperWin32::Close()
{
	if(serialPortHandle != NULL) {
...
bool CCU2SerialPortWrapperWin32::IsConnected() {
