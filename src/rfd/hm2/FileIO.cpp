...
bool FileIO::readStringFromFile(const std::string& filepath, std::string& result) 
{
	result.clear();
	bool done = false;
...
bool FileIO::writeStringToFile(const std::string& filepath, const std::string& str)
{
	if(filepath.empty()) {
		return false;
	}
