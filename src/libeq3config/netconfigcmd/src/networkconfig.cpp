...
void networkconfig::SetConfigAndAppendResultResponse(std::string & response, std::string & payload)
{
	...
	else
	{
		...
		snprintf(buffp, 20, "%u.%u.%u.%u", (unsigned char)payload.at(0), (unsigned char)payload.at(1), (unsigned char)payload.at(2), (unsigned char)payload.at(3));
		std::string tempip(buffp);
		this->config_ip = tempip;

		// Convert data to gateway address as string
		snprintf(buffp, 20, "%u.%u.%u.%u", (unsigned char)payload.at(4), (unsigned char)payload.at(5), (unsigned char)payload.at(6), (unsigned char)payload.at(7));
		std::string tempgateway(buffp);
		this->config_gateway = tempgateway;

		// Convert data to netmask as string
		snprintf(buffp, 20, "%u.%u.%u.%u", (unsigned char)payload.at(8), (unsigned char)payload.at(9), (unsigned char)payload.at(10), (unsigned char)payload.at(11));
		std::string tempnetmask(buffp);
		this->config_netmask = tempnetmask;

		// Convert data to primary dns server address as string
		snprintf(buffp, 20, "%u.%u.%u.%u", (unsigned char)payload.at(12), (unsigned char)payload.at(13), (unsigned char)payload.at(14), (unsigned char)payload.at(15));
		std::string tempdnssrv1(buffp);
		this->config_dnssrv1 = tempdnssrv1;

		// Convert data to secondary dns server address as string
		snprintf(buffp, 20, "%u.%u.%u.%u", (unsigned char)payload.at(16), (unsigned char)payload.at(17), (unsigned char)payload.at(18), (unsigned char)payload.at(19));
